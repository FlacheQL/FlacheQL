...
// parse and clean the newline chars, extra spaces, and slashes from the query
  ...
  it(query, variables, endpoint, headers = { "Content-Type": "application/graphql" }, options) {
      ...
      const key = this.cleanQuery(query);
      console.log('variables :', variables);
      // if an identical query comes in return the cached result
      if (this.cache[key]) {
          console.log('fetching from cache: ',this.cache[key]);
      ...
      else return this.fetchData(query, endpoint, headers, key);
      ...
      if (this.options.partialRetrieval) {
          ...
          Object.keys(variables).forEach(queryVariable => {
              ...
              if (this.queryCache[queryVariable]) {
                  this.queryCache[queryVariable][key] = variables[queryVariable]
              }
              else this.queryCache[queryVariable] = { [key]: variables[queryVariable] }
          })
          ...
          return this.fetchData(query, endpoint, headers, key);
      } else {
  ...
  fetchData(query, endpoint, headers, key) {
      return new Promise((resolve, reject) => {
          fetch(endpoint, {
              method: 'POST',
              headers,
          ...
          .then((res) => res.json())
          .then((res) => {
              this.cache[key] = this.flatten(res);
